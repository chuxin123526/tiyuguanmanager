...
public class UserVOValidator extends AbstractVOValidator {
	...
	public boolean validate(Object VOObject) throws VOTypeNotMatch {
		...
		lengthRange(PROPERTY_USERNAME, userVO.getUsername(), 6, 16,
				"用户名长度必须为6到16位");

		// 密码
		notEmpty(PROPERTY_PASSWORD, userVO.getPassword(), "密码不能为空");
		lengthRange(PROPERTY_PASSWORD, userVO.getPassword(), 6, 16,
				"密码长度必须为6到16位");
		...
		// 证件类型
		valueRange(PROPERTY_IDENTIFIER_TYPE, userVO.getIdentifierType(), 1, 3,
				"证件类型无效");
		...
		// 学号
		// 201311701407 长度必须为12位
		lengthRange(PROPERTY_STUDENT_NUMBER, userVO.getStudentNumber(), 12, 12,
				"学号长度必须为12位");
